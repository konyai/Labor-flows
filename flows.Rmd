...
Recent job leavers are those who lost their jobs and did not immediately start a new one. Time aggregation complicates matters, because they might have found a new one in a quarter.
...
ggplot() + geom_line(data=df[df$geo=="HU",],aes(x=time,y=rjl),color=col1) +
  geom_line(data=tran[tran$geo=="HU",],aes(x=time,y=E_U+E_I),color=col2)
```
...
df <- df %>% mutate(er=emp/pop,ur=une/pop,ir=ina/pop,esr=emps/pop,usr=unes/pop,rjlr=rjl/pop)
df <- df %>% select(geo,time,er,ur,ir,esr,usr,rjlr)
...
ggplot() + geom_line(data=df[df$geo=="HU",],aes(x=time,y=e_out),color=col1) +
  ...
  geom_line(data=df[df$geo=="HU",],aes(x=time,y=e_out_ui),color=col3)

ggplot() + geom_line(data=df[df$geo=="HU",],aes(x=time,y=e_out-e_out_ui),color=col1)
```
